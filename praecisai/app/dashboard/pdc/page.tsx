      ...
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
          {statCards.map((s) => {
            const Icon = s.icon;
            return (
              <div key={s.label} className="rounded-xl border border-[var(--caramel)] bg-[var(--surface-warm)] p-4">
                <div className="flex items-center gap-2 mb-2">
                  <Icon size={14} className={s.color} strokeWidth={1.75} />
                  <span className="font-body text-[11px] text-[var(--walnut)]">{s.label}</span>
                </div>
        ...
        <div className="rounded-xl border border-[var(--caramel)] bg-[var(--surface-warm)] p-6">
          ...
          <div
            className="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-[var(--caramel)] bg-[var(--sand)]/40 py-10 cursor-pointer hover:border-[var(--mahogany)] transition-colors"
            ...
            <div className="flex gap-2 flex-wrap">
              {['ALL', 'PENDING', 'CLEARED', 'BOUNCED'].map((s) => (
                <button
                  ...
                  className={cn(
                    'rounded-full px-3 py-1 font-body text-[12px] font-medium transition-colors border',
                    statusFilter === s
              ...
              <tbody className="divide-y divide-[var(--caramel)] text-[var(--dark-brown)]">
                {isLoading ? (
                  ...
                  <tr key={c.id} className="hover:bg-[var(--cream)] transition-colors">
                    <td className="px-4 py-2.5 font-medium">{c.party_name}</td>
                    <td className="px-4 py-2.5 text-[var(--walnut)]">{c.cheque_no}</td>
                    <td className="px-4 py-2.5 text-[var(--walnut)]">
                      {c.cheque_date ? new Date(c.cheque_date).toLocaleDateString('en-IN') : '—'}
                    </td>
                    <td className="px-4 py-2.5 font-medium">₹{Number(c.amount).toLocaleString('en-IN')}</td>
