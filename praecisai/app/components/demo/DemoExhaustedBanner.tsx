  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-4 rounded-2xl border border-[var(--caramel)] bg-[var(--surface-warm)] p-6 text-center shadow-sm sm:flex-row sm:text-left">
        <div>
          <h2 className="font-display flex items-center justify-center sm:justify-start gap-2 text-lg font-semibold text-[var(--dark-brown)]">
            <PartyPopper className="h-5 w-5 text-[var(--mahogany)]" />
            You&apos;ve experienced your free demo
          </h2>
          <p className="mt-1 font-body text-[14px] text-[var(--walnut)]">
            Ready to import your real outstanding report and start recovering cash automatically?
          </p>
        </div>
        <a 
          href="/#pricing"
          className="whitespace-nowrap rounded-xl bg-[var(--mahogany)] px-6 py-3 font-display text-[15px] font-semibold text-white transition-colors hover:bg-[var(--rust)] shadow-md"
        >
