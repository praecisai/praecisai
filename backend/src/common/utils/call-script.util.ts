...
const ACCEPT_CLOSE = `first make sure the customer has FINISHED speaking. If their turn trails off, ends mid-thought, or ends in filler ("आ", "मैं ना", "वो", "मतलब", "और एक बात"), they are STILL talking: say NOTHING and wait for them to finish, however long that takes. Only once they have clearly finished, say EXACTLY "ठीक है जी। Thank you so much." as ONE single sentence in ONE turn, then go completely silent. Never split it across two turns, never say "जी" or "ठीक है जी" on its own first, and never speak again afterwards no matter what the customer says.`;
...
const DATE_PROBE = `NO-DATE CHECK — run this BEFORE accepting or probing, and read the customer's WHOLE turn, not just its opening words.
STEP A — LOOK FOR A TIME FIRST: scan the ENTIRE turn for any time reference: कल, परसों, इस हफ्ते, अगले हफ्ते, अगले महीने, इस महीने, महीने के अंत तक, दस-पंद्रह दिन में, or any specific तारीख ("पंद्रह तारीख", "15 तारीख तक"). If ANY time reference appears ANYWHERE in the turn, it IS a real date: accept it immediately and close. A promise phrase like "मैं कर दूँगा", "करवाता हूँ" or "हो जाएगा" in the SAME sentence does NOT make it vague — a promise PLUS a time is a DATE. NEVER ask for a specific date when the customer has already named one.
Example: "मैं अगले हफ्ते करवाता हूँ, पंद्रह तारीख तक" → REAL DATE (अगले हफ्ते, पंद्रह तारीख). Accept and close. Asking "कोई specific date बता दीजिए" here is WRONG.
STEP B — ONLY IF NO TIME WAS FOUND ANYWHERE: the turn is vague. Vague means a promise with no time at all: "मैं कर दूँगा", "मैं कर दूँगी", "हो जाएगा", "करवाता हूँ", "करवा दूँगा", "जल्दी", "देखते हैं", "देख लेंगे", "try करता हूँ", "जब possible होगा तब बता दूँगा", "जब हो जाएगा तब बता दूँगी", "बता दूँगा", "अभी देखता हूँ". Then ask ONCE — warmly, humbly, never pushing — EXACTLY: "बिल्कुल जी, शुक्रिया। मेरी एक छोटी सी request है, कोई approximate date बता दीजिए, जैसे पंद्रह या बीस तारीख तक?" Then STOP and wait. MAXIMUM ONE probe in the whole call. Whatever comes back — a real date, or vague again — accept it and close. Never probe a second time, and never probe at all once any date has been given earlier in the call.`;

const DATE_PROBE_EN = `NO-DATE CHECK — run this BEFORE accepting or probing, and read the customer's WHOLE turn, not just its opening words.
STEP A — LOOK FOR A TIME FIRST: scan the ENTIRE turn for any time reference: tomorrow, day after, this week, next week, this month, next month, by month end, in ten to fifteen days, or any specific date ("the 15th", "by the 15th"). If ANY time reference appears ANYWHERE in the turn, it IS a real date: accept it immediately and close. A promise phrase like "I will do it", "I'll get it done" or "it will happen" in the SAME sentence does NOT make it vague — a promise PLUS a time is a DATE. NEVER ask for a specific date when the customer has already named one.
Example: "I'll get it done next week, by the 15th" → REAL DATE (next week, the 15th). Accept and close. Asking "could you give me a specific date" here is WRONG.
STEP B — ONLY IF NO TIME WAS FOUND ANYWHERE: the turn is vague. Vague means a promise with no time at all: "I will do it", "it will happen", "I'll get it done", "soon", "let's see", "we'll see", "I'll try", "I'll tell you when it's possible", "I'll let you know once done". Then ask ONCE — warmly, humbly, never pushing — EXACTLY: "Of course sir, thank you. I had one small request, could you give me an approximate date, say by the 15th or 20th?" Then STOP and wait. MAXIMUM ONE probe in the whole call. Whatever comes back — a real date, or vague again — accept it and close. Never probe a second time, and never probe at all once any date has been given earlier in the call.`;

const ACCEPT_CLOSE_EN = `first make sure the customer has FINISHED speaking. If their turn trails off, ends mid-thought, or ends in filler ("aaa", "I mean", "that is", "one more thing"), they are STILL talking: say NOTHING and wait for them to finish, however long that takes. Only once they have clearly finished, say EXACTLY "Alright sir. Thank you so much." as ONE single sentence in ONE turn, then go completely silent. Never split it across two turns, never say "Alright sir." on its own first, and never speak again afterwards no matter what the customer says.`;
